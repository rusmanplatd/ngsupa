...
@Component({
  ...
  template: `
    ...
    <ng-template #panelTemplate>
      <div
        class="select-panel rounded-2xl border border-[var(--glass-border)] bg-[var(--glass-bg-thick)] backdrop-blur-xl overflow-hidden"
  ...
  styles: `
    ...
    .select-panel {
      animation: select-panel-in 0.22s var(--ease-default);
    }
...
export class SelectComponent implements OnDestroy, ControlValueAccessor {
  ...
  readonly inputId = input('select-' + Math.random().toString(36).slice(2, 9));
  ...
  protected readonly filteredFlatList = computed<SelectOption[]>(() => {
    if (this.hasGroups()) {
  ...
  protected readonly hasValue = computed(() => this.internalValues().length > 0);

  protected readonly selectedOption = computed(() => {
    const vals = this.internalValues();
  ...
  /** The label floats up when there is a value or the panel is open */
  ...
  protected selectOption(option: SelectOption): void {
    ...
    if (this.multiple()) {
      this.internalValues.update((vals) => {
